...
If the determinant of the coefficient matrix D = 0,
	then the system is either dependent or inconsistent.
This will depend on Dx, Dy, and Dz. If they are all zero, 
	then the system is dependent. 
If at least one of these is nonzero, then it is inconsistent.
...
class Matriks {
    ...
    void TulisMatriks() {
        for (int i = 1; i <= this.NRowEff; i++) {
            for (int j = 1; j <= this.NColEff; j++) {
                System.out.printf("%.10f\t", this.Mat[i][j]);
            }
            System.out.println();
        }
    }
